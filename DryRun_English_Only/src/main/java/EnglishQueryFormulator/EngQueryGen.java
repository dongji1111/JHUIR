package EnglishQueryFormulator;

import EnglishUtils.EngQueryParserBETTER;
import EnglishUtils.EngRequestBETTER;
import EnglishUtils.EngTaskBETTER;
...
public class EngQueryGen {
    public Properties prop;
    public Logger logger;
    public String queryFilename;
    public EngQueryParserBETTER engQueryParserBETTER;
    public List<EngTaskBETTER> queries;
    ...
    private List<EngTaskBETTER> constructQueries() { // Loading the TREC format of queries
        return engQueryParserBETTER.queryFileParse();
    }

    public EngQueryGen(Properties prop, String indexDirPath, String queryFilename, String outputWriteFile, String searcherLogFilename) throws IOException {
        this.prop = prop;
        ...
        this.engQueryParserBETTER = new EngQueryParserBETTER(this.prop, queryFilename, this.logger);
        ...
        fieldToSearch = prop.getProperty("ENG_FIELD_TO_SEARCH", "TEXT");
    }
    ...
    public Document findDocumentContent(String docID) throws IOException {
        int internalLuceneID = findLuceneDocID(this.indexReader, prop.getProperty("ENG_FIELD_ID"), docID);
    ...
    private ArrayList<String> queryGenerator(EngRequestBETTER engRequestBETTER, ArrayList<String> taskDocs) throws IOException, QueryNodeException {
        ...
        for (String reqExt: engRequestBETTER.req_extr) {
        ...
        for (String docID: engRequestBETTER.req_docs) {
            documentContent = findDocumentContent(docID);
            if (documentContent == null) continue;

            // adding entities
            reqDocEntityList = extractEntitiesFromDocumentText(docID, documentContent.get(prop.getProperty("ENG_FIELD_BODY")));
            if (reqDocEntityList.size() == 0) {
                logger.info("--- MainSearcher : None entities found for req - " + engRequestBETTER.req_num + " - with docID - " + docID);
            } else {
            ...
            // adding text
            try {
                reqDocText = documentContent.get(this.prop.getProperty("ENG_FIELD_BODY"));
        ...
        for (String docID: taskDocs) {
            ...
            // adding entities
            taskDocEntityList = extractEntitiesFromDocumentText(docID, documentContent.get(prop.getProperty("ENG_FIELD_BODY")));
            ...
            // adding text
            try {
                taskDocText = documentContent.get(this.prop.getProperty("ENG_FIELD_BODY"));
        ...
        try {
            ...
            logger.warning("--- queryGenerator - MainSearcher : Formulated Query for queryID : " + engRequestBETTER.req_num + " : exceeded max-lucene QueryParser length, so truncating !");
...
//            }
    ...
    public static void main(String[] args) throws IOException, QueryNodeException {
        ...
        args[1] = "/Users/Norla/IdeaProjects/CasterCLIR/English-Indexing/ENGLISH_INDEX_DIR";
//        args[1] = "/Users/ramraj/better-ir/English-Turkey-run/Eval/environment-setup/English-Indexing/ENGLISH_INDEX_DIR";
//        args[1] = "/Users/ramraj/Projects/better-ir/English-Turkey-run/Submissions/BuildingDocker/Eval-autoIR/scratch/JHI_IR_English_IndexDir";
        args[2] = "app/dry-run-topics.final.v2.json";
        args[3] = "scratch/output-dryrun.json";
        args[4] = "scratch/log-dryrun.log";

        Properties prop = new Properties();
        prop.load(new FileReader(args[0]));
        EngQueryGen engQueryGen = new EngQueryGen(prop, args[1], args[2], args[3], args[4]);
        engQueryGen.mainEngQueryConstruction();
    }
}
